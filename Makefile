...
LIB_INCLUDES = -I"$(INCL_GLFW_DIR)" -I"$(INCL_DIR)"
else
LIB_GLFW = $$(pkg-config --libs glfw3)
LIB_INCLUDES = -I"$(INCL_DIR)"
endif
LIBS = $(LIB_GLFW)
...
COMPATIBILITY_HH = $(SRC_DIR)/compatibility.hh
CONFIGURATIONS_HH = $(SRC_DIR)/configurations.hh
CORE_HH = $(SRC_DIR)/core.hh
CORE_CC = $(SRC_DIR)/core.cc
CORE_OBJ = $(OBJ_DIR)/core.o
DEBUG_HH = $(SRC_DIR)/debug.hh
GEOMETRY_HH = $(SRC_DIR)/geometry.hh
MAIN_CC = $(SRC_DIR)/main.cc
MAIN_OBJ = $(OBJ_DIR)/main.o
RUNTIME_HH = $(SRC_DIR)/runtime.hh
RUNTIME_CC = $(SRC_DIR)/runtime.cc
RUNTIME_OBJ = $(OBJ_DIR)/runtime.o
WINDOW_HH = $(SRC_DIR)/window.hh
...
$(CORE_OBJ): $(CORE_CC) $(CORE_HH) $(CONFIGURATIONS_HH) $(DEBUG_HH)
	$(CXX) -c -o $@ $< $(WARNS) $(DEFINES) $(OPTIMIZE) $(CXX_STD) $(LIB_INCLUDES)

$(RUNTIME_OBJ): $(RUNTIME_CC) $(RUNTIME_HH) $(CONFIGURATIONS_HH) $(CORE_HH) $(GEOMETRY_HH) $(DEBUG_HH)
	$(CXX) -c -o $@ $< $(WARNS) $(DEFINES) $(OPTIMIZE) $(CXX_STD) $(LIB_INCLUDES)

$(WINDOW_OBJ): $(WINDOW_CC) $(WINDOW_HH) $(COMPATIBILITY_HH) $(CONFIGURATIONS_HH) $(DEBUG_HH)
	$(CXX) -c -o $@ $< $(WARNS) $(DEFINES) $(OPTIMIZE) $(CXX_STD) $(LIB_INCLUDES)
